...
from sqlalchemy import JSON, ForeignKey, func, Enum as SQLEnum
...
class TransactionsModel(BaseModel):
    ...
    transaction_type: Mapped[TransactionTypeEnum] = (
        mapped_column(SQLEnum(TransactionTypeEnum, values_callable=lambda enum_cls: [e.value for e in enum_cls])))
    ...
    created_at: Mapped[datetime] = mapped_column(server_default=func.now()) # server_default - чтобы подсчет выполнялся БД
    ...
    @validates("amount")
    def validate_amount(self, _: str, amount: int) -> int: # _ is the name of the column, here it is 'amount'
        if amount < 0:
