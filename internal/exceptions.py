...
class ForeignKeyException(BaseException):
    detail = "Это категория не принадлежит этому пользователю"
...
class ForeignKeyHTTPException(BaseHTTPException):
    status_code = 409
    detail = (
        "Проверьте правильность введенных данных: либо этому пользователю "
        "не принадлежит данная категория, либо этой категории не принадлежит указанный тип операции"
    )
